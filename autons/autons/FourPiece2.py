...
class FourPiece2(Auton):
    ...
    def run(self):
        ...
        # Grab third
        self.drive(velocity = Vector(-0.7, 0, self.subsystems.gyro.calculate(0)), duration = 0.5)
        self.drive(velocity = Vector(0, 0, self.subsystems.gyro.calculate(0)), duration = 0.3, brake = True)
        self.get_note()
        self.drive(velocity = Vector(0.9, 0, self.subsystems.gyro.calculate(0)), duration = 0.4)
        self.drive(velocity = Vector(0, 0, 0), duration = 0.3, brake = True)
